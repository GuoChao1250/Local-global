...
#%%
threshold=100e-6
Dev_1 = np.empty((30, 2, 64, 114))
STD_1 = np.empty((30, 2, 64, 114))
Dev_2 = np.empty((30, 2, 64, 114))
STD_2 = np.empty((30, 2, 64, 114))
...
for isSub in [0]:#range(len(Listing)):     
    ...
    for istype in range(len(Cond_list)):
        ...
        #  Extracting Epochs     
        local_std_1, local_dev_1  = extract_pairwise_mark(Full_epochs,std_mark='s11',dev_mark='s22',mark_step=1,event_id=event_id,threshold=threshold)
        local_std_2, local_dev_2  = extract_pairwise_mark(Full_epochs,std_mark='s1',dev_mark='s2',mark_step=1,event_id=event_id,threshold=threshold)    

        I_num_trials[isSub, istype] = local_std_1.shape[0]
        II_num_trials[isSub, istype] = local_std_2.shape[0]
        
        Dev_1[isSub, istype, :, :] = np.mean(local_dev_1,axis=0)
        STD_1[isSub, istype, :, :] = np.mean(local_std_1,axis=0)
        Dev_2[isSub, istype, :, :] = np.mean(local_dev_2,axis=0)
        STD_2[isSub, istype, :, :] = np.mean(local_std_2,axis=0)    
        # 清理内存
        del raw
times = epochs.times
# %%
test_data1 = [STD_1,STD_2]
test_data2 = [Dev_1,Dev_2]
significant_points = np.empty((len(Cond_list), len(test_data1), 64, 114))
T_value = np.empty((len(Cond_list), len(test_data1), 64, 114))
for i in range(len(test_data1)):
    data1 = test_data2[i]-test_data1[i]
    for istype in range(len(Cond_list)):
...
for istype in range(len(condition_type)):
    ...
    for index in true_indices:
        ...
        positive_windows = contin_significant_window(negative_mask & x_range_mask & time_window_std, window_size)
    ...
    ax.set_title(f"{condition_type[istype]}: Local I", fontsize=24, fontweight='bold')
    ...
    # plt.show()
    save_path = (
    f"./Fig/waveform/Local_effect/frequency/"
...
for istype in range(len(condition_type)):
    ...
    ax.plot(x, np.full(len(x), start2), color='black', linewidth=0.5)
    ax.set_title(f"{condition_type[istype]}: Local II", fontsize=24, fontweight='bold')
    ax.set_xlabel("Time(s)", fontsize=20)
    ax.set_ylabel("Amplitude(μV)", fontsize=20)
    ax.set_xlim(-0.1, 0.35)
    ax.set_ylim(y1, y2)
    ...
    ax.set_xticklabels([f"{tick:.1f}".rstrip("0").rstrip(".") 
                            if tick != 0 else "0" for tick in [0, 0.1, 0.2, 0.3]],fontsize=16, fontweight='bold')   
    ax.set_yticks([-16e-6, -12e-6, -8e-6, -4e-6, 0e-6,  4e-6,  8e-6, 12e-6, 16e-6])
    ax.set_yticklabels([f"{tick * 1e6:.1f}"[:-2] for tick in ax.get_yticks()], fontsize=16, fontweight='bold')
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
    legend = ax.legend(loc='upper left', fontsize=20, frameon=False)        
    ...
    # plt.show()
    save_path = (
    f"./Fig/waveform/Local_effect/numerical/"
    f"{condition_type[istype]}_{chan_names}.png"
    )      
    plt.savefig(save_path, bbox_inches='tight')        
# %%
...
np.savez(os.path.join(save_file_path, 'Local_effect.npz'), Dev_1=Dev_1, STD_1=STD_1, Dev_2=Dev_2
